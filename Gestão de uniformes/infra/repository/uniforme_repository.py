...
class UniformeRepository:
    ...
    @staticmethod
    def select_uniforme_by_name(nome_uniforme):
        with DBConnectionHandler() as db:
            uniforme = db.session.query(Uniforme).filter(Uniforme.nome == nome_uniforme).first()
            return uniforme
    ...
    def delete_uniforme(uniforme):
        with DBConnectionHandler() as db:
            db.session.query(Uniforme).filter(Uniforme.id == uniforme.id).update({'nome': False})
            db.session.commit()
